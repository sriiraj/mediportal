<!DOCTYPE>
<html>
<head>
	<title>Home - MediPortal</title>
</head>
<body>
<div align="center">
                        <td width="70%" align="center">
                             <div>
                                        <h1>Mr. XYZ</h1>
                                            <table width="100%">
                                                <tr>
                                                    <td width="60%">
                                                        <table width="100%">
                                                            <tr>
                                                                <td>
                                                                    <table width="100%">
                                                                        <td width="10%">&nbsp;</td>
                                                                        <td width="30%"><strong>Username</strong></td>
                                                                        <td><strong>:</strong></td>
                                                                        <td>user</td>
                                                                    </table>
                                                                </td>
                                                            </tr>
                                                            <tr><td><hr/></td></tr>
                                                            <tr>
                                                                <td>
                                                                    <table width="100%">
                                                                        <td width="10%">&nbsp;</td>
                                                                        <td width="30%"><strong>Gender</strong></td>
                                                                        <td><strong>:</strong></td>
                                                                        <td>Male</td>
                                                                    </table>
                                                                </td>
                                                            </tr>
                                                            <tr><td><hr/></td></tr>
                                                            <tr>
                                                                <td>
                                                                    <table width="100%">
                                                                        <td width="10%">&nbsp;</td>
                                                                        <td width="30%"><strong>Email</strong></td>
                                                                        <td><strong>:</strong></td>
                                                                        <td>xyz@example.com</td>
                                                                    </table>
                                                                </td>
                                                            </tr>
                                                            <tr><td><hr/></td></tr>
                                                            <tr>
                                                                <td>
                                                                    <table width="100%">
                                                                        <td width="10%">&nbsp;</td>
                                                                        <td width="30%"><strong>Blood Group</strong></td>
                                                                        <td><strong>:</strong></td>
                                                                        <td>B+</td>
                                                                    </table>
                                                                </td>
                                                            </tr>
                                                            <tr><td><hr/></td></tr>
                                                            <tr>
                                                                <td>
                                                                    <table width="100%">
                                                                        <td width="10%">&nbsp;</td>
                                                                        <td width="30%"><strong>BMDC Reg. No.</strong></td>
                                                                        <td><strong>:</strong></td>
                                                                        <td>B5484S5455A5</td>
                                                                    </table>
                                                                </td>
                                                            </tr>
                                                            <tr><td><hr/></td></tr>
                                                            <tr>
                                                                <td>
                                                                    <table width="100%">
                                                                        <td width="10%">&nbsp;</td>
                                                                        <td width="30%"><strong>Phone</strong></td>
                                                                        <td><strong>:</strong></td>
                                                                        <td>01700000000</td>
                                                                    </table>
                                                                </td>
                                                            </tr>
                                                            <tr><td><hr/></td></tr>
                                                            <tr>
                                                                <td>
                                                                    <table width="100%">
                                                                        <td width="10%">&nbsp;</td>
                                                                        <td width="30%"><strong>Date of Birth</strong></td>
                                                                        <td><strong>:</strong></td>
                                                                        <td>19/9/1998</td>
                                                                    </table>
                                                                </td>
                                                            </tr>
                                                            <tr><td><hr/></td></tr>
                                                            <tr>
                                                                <td>
                                                                    <table width="100%">
                                                                        <td width="10%">&nbsp;</td>
                                                                        <td width="30%"><strong>Country</strong></td>
                                                                        <td><strong>:</strong></td>
                                                                        <td>Bangladesh</td>
                                                                    </table>
                                                                </td>
                                                            </tr>
                                                            <tr><td><hr/></td></tr>
                                                            <tr>
                                                                <td>
                                                                    <table width="100%">
                                                                        <td width="10%">&nbsp;</td>
                                                                        <td width="30%"><strong>Chamber</strong></td>
                                                                        <td><strong>:</strong></td>
                                                                        <td><p>XYZ Hospital, XYZ<br>12/5 A, BCD<br>ABC Hospital, ABC<br>7/2 F, BXD, Dhk</p></td>
                                                                    </table>
                                                                </td>
                                                            </tr>
                                                            <tr><td><hr/></td></tr>

                                                            <tr>
                                                    <td>
                                                        <table width="100%">
                                                            <td width="10%"></td>
                                                            <td width="30%"><strong>Education</strong></td>
                                                            <td><strong>:</strong></td>
                                                            <td>
                                                                    <table>
                                                                        <tr>
                                                                            <td>Degree Name</td>
                                                                            <td>:</td>
                                                                            <td>MBBS</td>
                                                                        </tr>
                                                                        <tr>
                                                                            <td>Passed Year</td>
                                                                            <td>:</td>
                                                                            <td>2015</td>
                                                                        </tr>
                                                                        <tr>
                                                                            <td>University</td>
                                                                            <td>:</td>
                                                                            <td>DMC</td>
                                                                        </tr>
                                                                        <tr>
                                                                            <td>Description</td>
                                                                            <td>:</td>
                                                                            <td>Bla bla bla bla</td>
                                                                        </tr>
                                                              </table>                                                         
                                                            </td>
                                                        </table>
                                                    </td>
                                                </tr>
                                                        <tr><td><hr/></td></tr>
                                                        </table>
                                                    </td>
                                                    <td width="40%">
                                                        <table align="center">
                                                            <tr>
                                                                <td align="center"><img src="images/usericon.png"/></td>
                                                            </tr>
                                                        </table>
                                                    </td>
                                                </tr>
												<tr>
													<td align="center">
													 <button onclick="goBack()">Go back</button>
                                            <script>
                                                function goBack() {
                                                    window.history.back();
                                                }
                                            </script>		
													</td>
												</tr>
                                            </table>     
                                    </div>
                        </td></div>
                    </table>
                </div>
            </td>
        </tr>
        <tr>
        	<td>
        		<div>
                    <table align="center">
                        <td>&copy;2018 MediPortal@All rights reserved.</td>
                    </table>
                </div>
            </td>
        </tr>
    </table>
</body>
</html>